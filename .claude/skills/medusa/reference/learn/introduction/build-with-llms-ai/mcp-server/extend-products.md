# Extend Products with the MCP Server

In this chapter, you'll learn how to use the [Medusa MCP server](https://docs.medusajs.com/learn/introduction/build-with-llms-ai/mcp-server) to extend Medusa's Product Module with custom data and features.

## What is the Extend Products Tool?

The Medusa MCP server exposes a `get_product_custom_data_guide` tool that returns a complete, step-by-step implementation guide for attaching custom data to Medusa's [Product Module](https://docs.medusajs.com/resources/commerce-modules/product) data models: `product`, `productVariant`, and `productOption`.

You describe the data you want to attach, and your AI agent produces the extension: the custom data is stored, it's associated with products through a [module link](https://docs.medusajs.com/learn/fundamentals/module-links), it stays in sync as products are created, updated, and deleted, it can be read through the existing product APIs, and merchants can optionally manage it from the Medusa Admin.
...
After [connecting to the Medusa MCP server](https://docs.medusajs.com/learn/introduction/build-with-llms-ai/mcp-server#connect-to-the-medusa-mcp-server), ask your AI agent for the product extension you want. Your agent infers the name of the data from your request, calls the tool, and follows the returned guide.

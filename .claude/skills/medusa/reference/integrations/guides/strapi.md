...
- [Node.js v20.19.0+ or v22.12.0+ (Versions supported by Strapi)](https://nodejs.org/en/download)
...
```json title="src/api/product/content-types/product/schema.json"
{
  "kind": "collectionType",
...
```ts title="src/api/product/content-types/product/lifecycles.ts"
export default {
...
```ts title="src/api/product/controllers/product.ts"
...
```ts title="src/api/product/services/product.ts"
import { factories } from "@strapi/strapi"
...
```ts title="src/api/product/routes/product.ts"
import { factories } from "@strapi/strapi"
...
```json title="src/api/product-variant/content-types/product-variant/schema.json"
{
  "kind": "collectionType",
...
```ts title="src/api/product-variant/controllers/product-variant.ts"
import { factories } from "@strapi/strapi"
...
```ts title="src/api/product-variant/services/product-variant.ts"
import { factories } from "@strapi/strapi"
...
```ts title="src/api/product-variant/routes/product-variant.ts"
import { factories } from "@strapi/strapi"
...
```json title="src/api/product-option/content-types/product-option/schema.json"
{
  "kind": "collectionType",
...
```ts title="src/api/product-option/content-types/product-option/lifecycles.ts"
export default {
  async beforeDelete(event) {
    const { where } = event.params
...
```ts title="src/api/product-option/controllers/product-option.ts"
import { factories } from "@strapi/strapi"
...
```ts title="src/api/product-option/services/product-option.ts"
import { factories } from "@strapi/strapi"
...
```ts title="src/api/product-option/routes/product-option.ts"
import { factories } from "@strapi/strapi"
...
```json title="src/api/product-option-value/content-types/product-option-value/schema.json"
{
  "kind": "collectionType",
...
```ts title="src/api/product-option-value/controllers/product-option-value.ts"
import { factories } from "@strapi/strapi"
...
```ts title="src/api/product-option-value/services/product-option-value.ts"
import { factories } from "@strapi/strapi"
...
```ts title="src/api/product-option-value/routes/product-option-value.ts"
import { factories } from "@strapi/strapi"
...
A [module](https://docs.medusajs.com/docs/learn/fundamentals/modules) is a reusable package that provides functionalities related to a single feature or domain. Medusa integrates the module into your application without implications or side effects on your setup.
...
A [loader](https://docs.medusajs.com/docs/learn/fundamentals/modules/loaders) is an asynchronous function that runs when the Medusa server starts. Loaders are useful for setting up connections to third-party services and reusing those connections throughout your module.
...
```ts title="src/modules/strapi/loaders/init-client.ts"
...
1. `container`: The [module container](https://docs.medusajs.com/docs/learn/fundamentals/modules/container) that allows you to resolve and register module and Framework resources.
...
```ts title="src/modules/strapi/service.ts"
...
You resolve the [Logger](https://docs.medusajs.com/docs/learn/debugging-and-testing/logging) and the Strapi client that you registered in the loader. You also store the module options for later use.
...
```ts title="src/modules/strapi/index.ts"
...
```ts title="src/links/product-strapi.ts"
...
1. [Workflows](https://docs.medusajs.com/docs/learn/fundamentals/workflows) that implement the logic to create product data in Strapi.
2. A [subscriber](https://docs.medusajs.com/docs/learn/fundamentals/events-and-subscribers) that listens to the product creation event in Medusa and triggers the workflow.
...
```ts title="src/workflows/steps/create-options-in-strapi.ts"
...
```ts title="src/workflows/steps/create-option-values-in-strapi.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { STRAPI_MODULE } from "../../modules/strapi"
import StrapiModuleService, { Collection } from "../../modules/strapi/service"
...
```ts title="src/workflows/steps/update-product-option-values-metadata.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
...
```ts title="src/workflows/create-options-in-strapi.ts"
import { 
  createWorkflow, 
  WorkflowResponse,
  transform,
...
1. Retrieve the product options in Medusa using the `useQueryGraphStep`.
   - This step uses [Query](https://docs.medusajs.com/docs/learn/fundamentals/module-links/query) to retrieve data in Medusa across modules.
2. Prepare the option data to create using [transform](https://docs.medusajs.com/docs/learn/fundamentals/workflows/variable-manipulation).
...
In a workflow, you can't manipulate data because Medusa stores an internal representation of the workflow on application startup. Learn more in the [Data Manipulation](https://docs.medusajs.com/docs/learn/fundamentals/workflows/variable-manipulation) documentation.
...
```ts title="src/workflows/steps/upload-images-to-strapi.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { STRAPI_MODULE } from "../../modules/strapi"
...
```ts title="src/workflows/steps/create-variants-in-strapi.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { STRAPI_MODULE } from "../../modules/strapi"
import StrapiModuleService, { Collection } from "../../modules/strapi/service"
...
```ts title="src/workflows/steps/update-product-variants-metadata.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { Modules } from "@medusajs/framework/utils"
...
```ts title="src/workflows/create-variants-in-strapi.ts"
import { 
  createWorkflow, 
  WorkflowResponse,
  transform,
...
1. Acquire a [lock](https://docs.medusajs.com/resources/infrastructure-modules/locking) to prevent concurrent creation of variants for the same product. This is necessary to handle both the product and variant creation events without duplications.
2. Retrieve the product variants in Medusa using the `useQueryGraphStep`.
3. Check if the product has been created in Strapi using [when](https://docs.medusajs.com/docs/learn/fundamentals/workflows/conditions). If so, you:
...
In a workflow, you can't perform steps based on conditions because Medusa stores an internal representation of the workflow on application startup. Learn more in the [Conditions in Workflows](https://docs.medusajs.com/docs/learn/fundamentals/workflows/conditions) documentation.
...
- [retrieveFromStrapiStep](#retrieveFromStrapiStep): Retrieve existing product options from Strapi.
- [releaseLockStep](https://docs.medusajs.com/references/medusa-workflows/steps/releaseLockStep): Release the acquired lock
...
```ts title="src/workflows/steps/retrieve-from-strapi.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { STRAPI_MODULE } from "../../modules/strapi"
import StrapiModuleService, { Collection } from "../../modules/strapi/service"
...
```ts title="src/workflows/steps/update-product-in-strapi.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { STRAPI_MODULE } from "../../modules/strapi"
import StrapiModuleService, { Collection } from "../../modules/strapi/service"
...
```ts title="src/workflows/steps/create-product-in-strapi.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { STRAPI_MODULE } from "../../modules/strapi"
import StrapiModuleService, { Collection } from "../../modules/strapi/service"
...
```ts title="src/workflows/create-product-in-strapi.ts"
import { 
  createWorkflow, 
  WorkflowResponse,
  transform,
...
1. Acquire a [lock](https://docs.medusajs.com/resources/infrastructure-modules/locking) to prevent concurrent creation of variants for the same product. This is necessary to handle both the product and variant creation events. Otherwise, variants might be created multiple times.
...
Refer to the [Subscribers documentation](https://docs.medusajs.com/docs/learn/fundamentals/events-and-subscribers) to learn more about subscribers.

To create the subscriber, create the file `src/subscribers/product-created-strapi-sync.ts` with the following content:

```ts title="src/subscribers/product-created-strapi-sync.ts"
import {
...
```ts title="src/workflows/steps/prepare-strapi-update-data.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
...
The `clearProductCacheStep` clears the product cache in Medusa to ensure that updated data is served to clients. This is necessary as you'll enable [caching](https://docs.medusajs.com/resources/infrastructure-modules/caching) later, which may cause stale data to be served to the storefront.

To create the step, create the file `src/workflows/steps/clear-product-cache.ts` with the following content:

```ts title="src/workflows/steps/clear-product-cache.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { Modules } from "@medusajs/framework/utils"
...
```ts title="src/workflows/steps/update-product-option-value.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { Modules } from "@medusajs/framework/utils"
...
```ts title="src/workflows/handle-strapi-webhook.ts"
import { 
  createWorkflow, 
  when,
  transform,
...
An [API route](https://docs.medusajs.com/docs/learn/fundamentals/api-routes) is an endpoint that exposes business logic and commerce features to clients.
...
```ts title="src/api/webhooks/strapi/route.ts"
...
To ensure that webhook requests are coming from your Strapi application, you'll add a [middleware](https://docs.medusajs.com/docs/learn/fundamentals/api-routes/middlewares) that validates the webhook requests.
...
1. Resolves the [API Key Module](https://docs.medusajs.com/resources/commerce-modules/api-key)'s service.
...
The [Caching Module](https://docs.medusajs.com/resources/infrastructure-modules/caching) is currently guarded by a feature flag. To enable it, add the feature flag and module in your `medusa-config.ts` file:
...
```ts title="src/lib/data/product.ts"
...
```ts title="src/lib/util/strapi.ts"
...
```tsx title="src/modules/products/components/product-actions/option-select.tsx"
import { HttpTypes } from "@medusajs/types"

...
Medusa's [Tax Module](https://docs.medusajs.com/resources/commerce-modules/tax) implements concepts and functionalities related to taxes, but delegates tax calculations to external services through Tax Module Providers.
...
```ts title="src/modules/avalara/types.ts"
...
1. An object with the dependencies to resolve from the [Module's container](https://docs.medusajs.com/docs/learn/fundamentals/modules/container).
...
You'll test the Avalara integration using the [Next.js Starter Storefront](https://docs.medusajs.com/resources/nextjs-starter) that you installed earlier. You'll proceed through checkout and verify that taxes are calculated using Avalara.
...
2. Create a [workflow](https://docs.medusajs.com/docs/learn/fundamentals/workflows) that creates a transaction in Avalara for an order.
3. Create a [subscriber](https://docs.medusajs.com/docs/learn/fundamentals/events-and-subscribers) that listens to the `order.placed` event and triggers the workflow.
...
- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Retrieve order's details.
- [createTransactionStep](#createTransactionStep): Create a transaction for order.
- [updateOrderWorkflow](https://docs.medusajs.com/references/medusa-workflows/updateOrderWorkflow): Save transaction code in order metadata.
...
```ts title="src/workflows/steps/create-transaction.ts"
...
So far, you retrieve the order's details using the `useQueryGraphStep`. It uses [Query](https://docs.medusajs.com/docs/learn/fundamentals/module-links/query) under the hood to retrieve data across modules.
...
- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Retrieve product's details.
- [createItemStep](#createItemStep): Create Avalara item for the product.
- [updateProductsWorkflow](https://docs.medusajs.com/references/medusa-workflows/updateProductsWorkflow): Save Avalara item ID in product metadata.
...
```ts title="src/workflows/create-product-item.ts"
...
```ts title="src/workflows/steps/update-item.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import AvalaraTaxModuleProvider from "../../modules/avalara/service"
...
```ts title="src/workflows/update-variant-item.ts"
...
`when-then` allows you to run steps based on conditions during execution. Learn more in the [Conditions in Workflows](https://docs.medusajs.com/docs/learn/fundamentals/workflows/conditions) documentation.
...
- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Retrieve the product's details.
...
```ts title="src/workflows/steps/delete-item.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import AvalaraTaxModuleProvider from "../../modules/avalara/service"
...
```ts title="src/workflows/delete-product-item.ts"
...
If you're new to Medusa, check out the [main documentation](https://docs.medusajs.com/docs/learn) for a more in-depth understanding of the concepts used in this guide.

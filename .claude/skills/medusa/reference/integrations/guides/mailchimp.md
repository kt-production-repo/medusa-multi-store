...
Medusa's [Notification Module](https://docs.medusajs.com/resources/infrastructure-modules/notification) allows you to customize Medusa's infrastructure to send notifications using the third-party provider that fits your business needs, such as [Mailchimp](https://mailchimp.com/).
...
To integrate third-party services into Medusa, you create a custom [module](https://docs.medusajs.com/docs/learn/fundamentals/modules). A module is a reusable package with functionalities related to a single feature or domain.

Medusa's [Notification Module](https://docs.medusajs.com/resources/infrastructure-modules/notification) provides an interface to send notifications in your Medusa application. It delegates the actual sending of notifications to the underlying provider, such as Mailchimp.
...
- `container`: The [module's container](https://docs.medusajs.com/docs/learn/fundamentals/modules/container) that contains Framework resources available to the module. You don't need to use it for this tutorial.
...
Learn about other properties in the object in the [Create Notification Module Provider](https://docs.medusajs.com/references/notification-provider-module#send) guide.
...
Learn about the object's properties in the [Create Notification Module Provider](https://docs.medusajs.com/references/notification-provider-module#send) guide.
...
- Create an [API route](https://docs.medusajs.com/docs/learn/fundamentals/api-routes) to subscribe customers. An API Route is an endpoint that exposes commerce features to external applications and clients, such as storefronts.
  - In the API route, you'll emit an event indicating that the customer is signing up for the newsletter.
- Create a [subscriber](https://docs.medusajs.com/docs/learn/fundamentals/events-and-subscribers) that listens to the event emitted by the API route. The subscriber will use Mailchimp to subscribe the customer to the newsletter audience list.
...
```ts title="src/api/store/newsletter/route.ts"
...
In the route handler, you use the [Medusa container](https://docs.medusajs.com/docs/learn/fundamentals/medusa-container) to resolve the [Event Module](https://docs.medusajs.com/resources/infrastructure-modules/event)'s service.
...
In the subscriber function, you resolve the [Notification Module](https://docs.medusajs.com/resources/infrastructure-modules/notification)'s service from the Medusa container. Then, you call the `createNotifications` method to create a notification with the following properties:
...
In this step, you'll customize the [Next.js Starter Storefront](https://docs.medusajs.com/resources/nextjs-starter) that you installed as part of the first step. You'll add a newsletter subscription form in the storefront's footer that allows customers to subscribe to the newsletter.
...
```ts title="src/lib/data/newsletter.ts"
"use server"
...
You use the [JS SDK](https://docs.medusajs.com/resources/js-sdk), which is already configured in the Next.js Starter Storefront, to send a `POST` request to the `/store/newsletter` API route. You pass the email address in the request body.
...
```tsx title="src/modules/layout/components/newsletter-form/index.tsx"
"use client"
...
The `toast` function is imported from [Medusa UI](https://docs.medusajs.com/ui), which requires adding `Toaster` component in the application's tree. So, import the `Toaster` component in the file `src/app/[countryCode]/(main)/layout.tsx`:
...
```tsx title="src/modules/layout/components/footer/index.tsx"
...
```ts title="src/workflows/send-newsletter.ts"
import { 
  createWorkflow, 
  when, 
  WorkflowResponse,
...
1. Use the [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep) to retrieve products created in the last 7 days. You retrieve the product's title, handle, thumbnail, and description fields.
2. Use the [when-then](https://docs.medusajs.com/docs/learn/fundamentals/workflows/conditions) utility to send the notification only if there are new products. `when` receives two parameters:
...
3. If the `when` condition is met, you use the [sendNotificationsStep](https://docs.medusajs.com/references/medusa-workflows/steps/sendNotificationsStep) to create a notification with the following properties:
...
To automate executing a task at a specified interval, you can create a [scheduled job](https://docs.medusajs.com/docs/learn/fundamentals/scheduled-jobs). A scheduled job is a background task that runs at a specified interval, such as every hour or every day.
...
- An asynchronous function that executes the job's logic. The function receives the [Medusa container](https://docs.medusajs.com/docs/learn/fundamentals/medusa-container) as a parameter.
...
1. Resolve the [Logger](https://docs.medusajs.com/docs/learn/debugging-and-testing/logging) utility from the Medusa container to log messages.
...
Also, if you didn't create any products in the past week, make sure to [create a few products in the Medusa Admin](https://docs.medusajs.com/user-guide/products/create).
...
- Send newsletters about other events, such as new collections or special offers. Refer to the [Events Reference](https://docs.medusajs.com/references/events) for the list of events you can listen to.

...
This tutorial is divided into two parts: this part that covers the backend and admin customizations, and a [storefront part](https://docs.medusajs.com/resources/recipes/ticket-booking/example/storefront) that covers the Next.js Starter Storefront customizations.
...
Medusa's [Framework](https://docs.medusajs.com/docs/learn/fundamentals/framework) facilitates customizing Medusa's core features for your specific use case, such as ticket booking.
...
- [Extend the Next.js Starter Storefront to allow booking tickets and choosing seats](https://docs.medusajs.com/resources/recipes/ticket-booking/example/storefront): This part of the tutorial is covered separately.
...
- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Retrieves the created venue with its rows.
...
4. Retrieve the created venue with its rows using the `useQueryGraphStep`.
   - This step uses [Query](https://docs.medusajs.com/docs/learn/fundamentals/module-links/query) under the hood. It allows you to retrieve data across modules.
...
`transform` allows you to access the values of data during execution. Learn more in the [Data Manipulation](https://docs.medusajs.com/docs/learn/fundamentals/workflows/variable-manipulation) documentation.
...
Refer to the [Query](https://docs.medusajs.com/docs/learn/fundamentals/module-links/query#request-query-configurations) documentation to learn more about query request configurations.
...
In the `VenuesPage` component, you'll display the list of venues in a [Data Table](https://docs.medusajs.com/ui/components/data-table) component.
...
- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Retrieve default store configuration that are useful for creating the Medusa product.
- [createInventoryItemsWorkflow](https://docs.medusajs.com/references/medusa-workflows/createInventoryItemsWorkflow): Creates inventory items for the Medusa product.
- [createProductsWorkflow](https://docs.medusajs.com/references/medusa-workflows/createProductsWorkflow): Creates the Medusa product and its variants.
...
- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Retrieve the created ticket product with its relations.
...
Make sure you have [default location and sales channel set up in your Medusa store](https://docs.medusajs.com/user-guide/settings/store).
...
In Medusa, you can set the price of a product variant in multiple currencies and regions. This allows you to sell products in different markets with localized pricing. Learn more in the [Pricing documentation](https://docs.medusajs.com/resources/commerce-modules/pricing/concepts).
...
Medusa implements cart operations in workflows. Specifically, you'll focus on the `addToCartWorkflow`. Medusa allows you to inject custom logic into workflows using [hooks](https://docs.medusajs.com/docs/learn/fundamentals/workflows/workflow-hooks).
...
You can test out the hook when you [customize the storefront](https://docs.medusajs.com/resources/recipes/ticket-booking/example/storefront).
...
- [completeCartWorkflow](https://docs.medusajs.com/references/medusa-workflows/completeCartWorkflow): Complete the cart using Medusa's default completeCartWorkflow
- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Retrieve the cart details
- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Retrieve existing ticket purchases to ensure idempotency
- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Retrieve the order details
...
```ts title="src/workflows/steps/create-ticket-purchases.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { 
  CartDTO, 
  CartLineItemDTO, 
...
You'll test out this API route when you [customize the storefront](https://docs.medusajs.com/resources/recipes/ticket-booking/example/storefront#update-cart-functions).

***

## Step 12: Storefront API Routes

To [customize the storefront](https://docs.medusajs.com/resources/recipes/ticket-booking/example/storefront) in the next part, you need two API routes that the storefront will consume:
...
- A [Notification Module Provider](https://docs.medusajs.com/resources/infrastructure-modules/notification) to handle sending emails. For example, [SendGrid](https://docs.medusajs.com/resources/infrastructure-modules/notification/sendgrid) or [Resend](https://docs.medusajs.com/resources/integrations/guides/resend).
...
- A [Subscriber](https://docs.medusajs.com/docs/learn/fundamentals/events-and-subscribers) that listens to the `order.placed` event and sends an email with the order details.
...
Make sure to set the `SENDGRID_API_KEY` and `SENDGRID_FROM` environment variables in your `.env` file, as explained in the [SendGrid Notification Module Provider guide](https://docs.medusajs.com/resources/infrastructure-modules/notification/sendgrid).
...
To test the order confirmation email, [customize the storefront](https://docs.medusajs.com/resources/recipes/ticket-booking/example/storefront) first. Then, start the Medusa application and the Next.js Starter Storefront.
...
You've now implemented the ticket booking functionality in the backend. Follow the [Ticket Booking Storefront](https://docs.medusajs.com/resources/recipes/ticket-booking/example/storefront) tutorial to customize the Next.js Starter Storefront for ticket booking.
...
- Handle [order events](https://docs.medusajs.com/references/events#order-events), such as `order.canceled`, to make changes to ticket purchases.

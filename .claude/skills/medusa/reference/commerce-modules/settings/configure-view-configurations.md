...
- [Medusa v2.18.0 or later](https://github.com/medusajs/medusa/releases/tag/v2.18.0)
- [View configurations feature flag enabled](../view-configurations/page.mdx#enable-the-feature-flag)

## How Column Configurations are Generated

The Settings Module discovers the entities in your application from the registered modules' [query graph](https://docs.medusajs.com/docs/learn/fundamentals/module-links/query), including your custom modules' data models. For each entity, it generates the column configurations that are used in its configurable data table in the Medusa Admin dashboard.
...
In this guide, the terms "entity" and "data model" are used interchangeably. They both refer to a [data model](https://docs.medusajs.com/docs/learn/fundamentals/data-models) defined in a module, such as a core data model like `Product` or a custom one like `Brand`.
...
- [A custom module with a data model defined](https://docs.medusajs.com/docs/learn/customization/custom-features/module)
...
- [An API route that lists the entity's records](https://docs.medusajs.com/docs/learn/customization/customize-admin/route#1-get-brands-api-route)

Next, you can show a configurable, view-aware data table for an entity in your admin customizations, such as a [custom UI route](https://docs.medusajs.com/docs/learn/fundamentals/admin/ui-routes).
...
If you don't need saved views or dynamic columns, use the [DataTable component](https://docs.medusajs.com/resources/admin-components/components/data-table) from `@medusajs/ui` instead. It's a stable, fully-supported table primitive.
...
|---|---|
|\`text\`|Shows the value as plain text.|
|\`number\`|Formats the value as a localized number.|
|\`currency\`|Formats the value as a price using the same row's |
|\`date\`|Formats the value as a date.|
|\`boolean\`|Shows the value as a Yes or No badge.|
|\`email\`|Shows the value as a |
|\`phone\`|Shows the value as a |
|\`url\`|Shows the value as an external link.|
|\`image\`|Shows the value as an image.|
|\`json\`|Shows the value as formatted JSON.|
|\`handle\`|Shows the value as a URL path prefixed with |
|\`count\`|Shows the number of items in the array specified in the entity configuration's |
|\`badges\`|Shows the items in the array specified in the entity configuration's |
|\`status\`|Shows a status pill resolved from the entity configuration's |
|\`name\`|Shows a full name from the |
|\`address\`|Shows a formatted address from the object at the entity configuration's |
|\`country\_code\`|Shows a country flag from the code at the entity configuration's |

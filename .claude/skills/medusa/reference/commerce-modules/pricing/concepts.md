...
The [Price](https://docs.medusajs.com/references/pricing/models/Price) data model represents a specific price for a resource. For example, the price of a product variant in the USD currency.
...
A [PriceSet](https://docs.medusajs.com/references/pricing/models/PriceSet) represents a collection of prices that are linked to a resource. For example, a product variant can have a price set that includes prices in multiple currencies, such as USD and EUR.
...
A [PriceList](https://docs.medusajs.com/references/pricing/models/PriceList) is a group of prices that are only enabled when their conditions and rules are satisfied. For example, you can apply special prices to customers in the VIP group.

When the conditions are met, the prices in the price list can override the default prices in a price set. Learn more in the [Price Calculation](https://docs.medusajs.com/resources/commerce-modules/pricing/price-calculation) guide.
...
For example, Medusa links a product variant from the [Product Module](https://docs.medusajs.com/resources/commerce-modules/product) to a price set that contains prices in multiple currencies. A variant's price set would be similar to the following:
...
The `Price` data model has a relation to the [PriceRule](https://docs.medusajs.com/references/pricing/models/PriceRule) data model that allows you to define prices that are applied based on specific conditions, such as the customer's region.

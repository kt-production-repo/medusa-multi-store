...
```ts title="src/modules/product-media/models/product-category-image.ts"
...
1. A [workflow](https://docs.medusajs.com/docs/learn/fundamentals/workflows) with steps that define the feature's business logic.
...
2. An async function that receives two parameters:
   ...
   - An object with properties including the [Medusa container](https://docs.medusajs.com/docs/learn/fundamentals/medusa-container), which is a registry of Framework and commerce tools accessible in the step.
...
```ts title="src/workflows/steps/create-category-images.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { PRODUCT_MEDIA_MODULE } from "../../modules/product-media"
import ProductMediaModuleService from "../../modules/product-media/service"
...
```ts title="src/workflows/create-category-images.ts"
import {
  createWorkflow,
  transform,
  when,
...
1. Check if any of the images to create is a thumbnail using [when-then](https://docs.medusajs.com/docs/learn/fundamentals/workflows/conditions).
...
To validate the body parameters of requests sent to the API route, apply a [middleware](https://docs.medusajs.com/docs/learn/fundamentals/api-routes/middlewares).
...
In the API route, you resolve [Query](https://docs.medusajs.com/docs/learn/fundamentals/module-links/query), which retrieves data across modules.
...
1. `uploadFilesMutation`: A mutation that uploads files using Medusa's existing API route for uploads. This will upload the images to the [configured File Module Provider](https://docs.medusajs.com/resources/infrastructure-modules/file#what-is-a-file-module-provider).
...
Images are uploaded to the [configured File Module Provider](https://docs.medusajs.com/resources/infrastructure-modules/file#what-is-a-file-module-provider). If you haven't configured one, images will be uploaded to the `static` folder in your Medusa project.
...
```ts title="src/workflows/steps/update-category-images.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { PRODUCT_MEDIA_MODULE } from "../../modules/product-media"
import ProductMediaModuleService from "../../modules/product-media/service"
...
```ts title="src/workflows/update-category-images.ts"
import {
  createWorkflow,
  WorkflowResponse,
  transform,
...
```ts title="src/workflows/steps/delete-category-images.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { PRODUCT_MEDIA_MODULE } from "../../modules/product-media"
import ProductMediaModuleService from "../../modules/product-media/service"
...
```ts title="src/workflows/delete-category-images.ts"
import {
  createWorkflow,
  WorkflowResponse,
  transform,
...
2. Prepare the file IDs using [transform](https://docs.medusajs.com/docs/learn/fundamentals/workflows/variable-manipulation).
...
In the last step, you'll update the [Next.js Starter Storefront](https://docs.medusajs.com/resources/nextjs-starter) to:
...
You can do this by creating a [read-only link](https://docs.medusajs.com/docs/learn/fundamentals/module-links/read-only). A read-only link allows you to retrieve data related to a model from another module without compromising [module isolation](https://docs.medusajs.com/docs/learn/fundamentals/modules/isolation).
...
```ts title="src/links/product-category-image.ts"
...
```ts title="src/lib/data/categories.ts"
...
```tsx title="src/modules/layout/components/megamenu/index.tsx"
...
```tsx title="src/modules/layout/templates/nav/index.tsx"
...
```tsx title="src/modules/categories/components/category-banner/index.tsx"
...
```tsx title="src/modules/categories/templates/index.tsx"

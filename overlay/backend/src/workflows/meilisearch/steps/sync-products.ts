...
export const syncProductsStep = createStep(
  ...
  async ({ products }: SyncProductsStepInput, { container }) => {
    ...
    const newProducts = products.filter(
      (product) =>
        !existingProducts.some((p) => p.id === product.id)
    )
  ...
  async (input, { container }) => {
    ...
    if (input.newProducts) {
      await meilisearchModuleService.deleteFromIndex(input.newProducts, "product")
    }

    if (input.existingProducts) {
      await meilisearchModuleService.indexData(
        input.existingProducts as unknown as Record<string, unknown>[],
        "product"
      )
    }
  }

...
const ProductVendorWidget = ({ data }: DetailWidgetProps<{ id: string }>) => {
  const { data: response, isLoading } = useQuery({
    queryFn: () =>
      sdk.client.fetch<ProductVendorResponse>(`/admin/products/${data.id}/vendor`),
    queryKey: ["product-vendor", data.id],
    enabled: !!data.id,
    retry: false,
  })

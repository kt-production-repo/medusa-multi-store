...
A custom auth provider can't change the shape of this route's response body. The provider's [authenticate method](https://docs.medusajs.com/references/auth/provider#authenticate) returns an [AuthenticationResponse](https://docs.medusajs.com/references/auth/interfaces/auth.AuthenticationResponse) object, but this route only uses its `success`, `authIdentity`, `location`, and `error` fields to build the response. Any other fields you return from the provider are ignored.

If you need to return a custom response body, create a [custom API route](https://docs.medusajs.com/docs/learn/fundamentals/api-routes) that calls the [Auth Module's authenticate method](https://docs.medusajs.com/references/auth/authenticate), then shape the response as you need.

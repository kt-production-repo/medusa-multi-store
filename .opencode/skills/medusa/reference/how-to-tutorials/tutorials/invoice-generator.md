...
When you install a Medusa application, you get a fully-fledged commerce platform with a Framework for customization. The Medusa application's commerce features are built around [Commerce Modules](https://docs.medusajs.com/resources/commerce-modules), which are available out-of-the-box. These features include order management capabilities.
...
```ts title="src/modules/invoice-generator/models/invoice-config.ts"
...
```ts title="src/modules/invoice-generator/models/invoice.ts"
import { model } from "@medusajs/framework/utils"
...
Since you need to create the default configurations when the Medusa application starts, you'll use a [loader](https://docs.medusajs.com/docs/learn/fundamentals/modules/loaders). A loader is a script in your module that Medusa runs on application startup.
...
```ts title="src/modules/invoice-generator/loaders/create-default-config.ts"
import {
  LoaderOptions,
...
The loader function accepts an object having the [module's container](https://docs.medusajs.com/docs/learn/fundamentals/modules/container) as a parameter. You can use this container to resolve Framework tools and the module's resources.
...
Refer to the [Loaders](https://docs.medusajs.com/docs/learn/fundamentals/modules/loaders) documentation to learn more about loaders.
...
1. A [workflow](https://docs.medusajs.com/docs/learn/fundamentals/workflows) with the business logic to manage invoice configurations.
2. An [API route](https://docs.medusajs.com/docs/learn/fundamentals/api-routes) that exposes the invoice configuration management functionality to clients.
3. A [settings page](https://docs.medusajs.com/docs/learn/fundamentals/admin/ui-routes#create-settings-page) in the Medusa Admin dashboard that allows admin users to manage the invoice configurations.
...
```ts title="src/workflows/steps/update-invoice-config.ts"
...
In the route handler, you resolve [Query](https://docs.medusajs.com/docs/learn/fundamentals/module-links/query) from the Medusa container. It allows you to retrieve data across modules.
...
Settings pages are [UI routes](https://docs.medusajs.com/docs/learn/fundamentals/admin/ui-routes#create-settings-page) created under the `/src/admin/routes/settings` directory. Medusa will then add the settings page under the Settings section of the Medusa Admin dashboard.
...
6. Define an `uploadLogo` function that uploads a logo using Medusa's [Upload API route](https://docs.medusajs.com/api/admin/uploads/upload-files).
...
Refer to the [Admin Components](https://docs.medusajs.com/resources/admin-components/components/forms) documentation to learn more about creating forms in the Medusa Admin dashboard.
...
- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Retrieve countries for billing and shipping addresses
...
```ts title="src/workflows/steps/get-order-invoice.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { INVOICE_MODULE } from "../../modules/invoice-generator"
...
```ts title="src/workflows/steps/generate-invoice-pdf.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { INVOICE_MODULE } from "../../modules/invoice-generator"
...
```ts title="src/workflows/generate-invoice-pdf.ts"
...
```ts title="src/api/admin/orders/[id]/invoices/route.ts"
...
Workflows and steps serialize their output. So, you must recreate the buffer from the serialized output using `Buffer.from`. Learn more in the [Constructor Constraints](https://docs.medusajs.com/docs/learn/fundamentals/workflows/constructor-constraints#returned-values) documentation.
...
A [widget](https://docs.medusajs.com/docs/learn/fundamentals/admin/widgets) is a React component that is injected into an existing admin page.

To create the widget, create the file `src/admin/widgets/order-invoice.tsx` with the following content:

```tsx title="src/admin/widgets/order-invoice.tsx"
...
```tsx title="src/modules/order/components/order-details/index.tsx"
"use client"
...
You can listen to events that occur in your Medusa application, such as when an order is placed, using [subscribers](https://docs.medusajs.com/docs/learn/fundamentals/events-and-subscribers). A subscriber is an asynchronous function that is executed whenever its associated event is emitted.
...
- Send an email using the [Notification Module](https://docs.medusajs.com/resources/infrastructure-modules/notification) with the order details and the invoice PDF attached.

#### Notification Module Provider to Use

Since the notification's channel is `email`, you need a Notification Module Provider that supports sending emails, such as [SendGrid](https://docs.medusajs.com/resources/infrastructure-modules/notification/sendgrid) or [Resend](https://docs.medusajs.com/resources/integrations/guides/resend).
...
```ts title="src/workflows/steps/update-invoices.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { InvoiceStatus } from "../../modules/invoice-generator/models/invoice"
import { INVOICE_MODULE } from "../../modules/invoice-generator"
...
Refer to the [Events Reference](https://docs.medusajs.com/references/events) for a complete list of events that Medusa emits.

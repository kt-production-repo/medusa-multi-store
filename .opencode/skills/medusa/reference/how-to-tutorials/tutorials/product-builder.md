...
Refer to the [Modules](https://docs.medusajs.com/docs/learn/fundamentals/modules) documentation to learn more.
...
Refer to the [Data Models](https://docs.medusajs.com/docs/learn/fundamentals/modules#1-create-data-model) documentation to learn more.
...
```ts title="src/modules/product-builder/models/product-builder.ts"
...
```ts title="src/modules/product-builder/models/product-builder-custom-field.ts"
import { model } from "@medusajs/framework/utils"
...
```ts title="src/modules/product-builder/models/product-builder-complementary.ts"
import { model } from "@medusajs/framework/utils"
import ProductBuilder from "./product-builder"
...
```ts title="src/modules/product-builder/models/product-builder-addon.ts"
import { model } from "@medusajs/framework/utils"
import ProductBuilder from "./product-builder"
...
Find all methods generated by the `MedusaService` in [the Service Factory](https://docs.medusajs.com/resources/service-factory-reference) reference.
...
```ts title="src/modules/product-builder/index.ts"
...
```ts title="src/links/product-builder-product.ts"
...
```ts title="src/links/product-builder-complementary-product.ts"
import ProductBuilderModule from "../modules/product-builder"
import ProductModule from "@medusajs/medusa/product"
import { defineLink } from "@medusajs/framework/utils"
...
```ts title="src/links/product-builder-addon-product.ts"
import ProductBuilderModule from "../modules/product-builder"
import ProductModule from "@medusajs/medusa/product"
import { defineLink } from "@medusajs/framework/utils"
...
- A [workflow](https://docs.medusajs.com/docs/learn/fundamentals/workflows) to create or update (upsert) product builder configurations.
- An [API route](https://docs.medusajs.com/docs/learn/fundamentals/api-routes) to expose the workflow's functionality to client applications.
...
- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Retrieve the product builder configuration, if it exists.
...
- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Retrieve the updated product builder configuration.
...
If you get a type error on resolving the Product Builder Module, run the Medusa application once with the `npm run dev` or `yarn dev` command to generate the necessary type definitions, as explained in the [Automatically Generated Types guide](https://docs.medusajs.com/docs/learn/fundamentals/generated-types).

```ts title="src/workflows/steps/create-product-builder.ts"
...
```ts title="src/workflows/steps/prepare-product-builder-custom-fields.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { PRODUCT_BUILDER_MODULE } from "../../modules/product-builder"
...
```ts title="src/workflows/steps/create-product-builder-custom-fields.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { PRODUCT_BUILDER_MODULE } from "../../modules/product-builder"
...
```ts title="src/workflows/steps/update-product-builder-custom-fields.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { PRODUCT_BUILDER_MODULE } from "../../modules/product-builder"
...
```ts title="src/workflows/steps/delete-product-builder-custom-fields.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { PRODUCT_BUILDER_MODULE } from "../../modules/product-builder"
...
```ts title="src/workflows/steps/prepare-product-builder-complementary-products.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { PRODUCT_BUILDER_MODULE } from "../../modules/product-builder"
...
```ts title="src/workflows/steps/create-product-builder-complementary-products.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { PRODUCT_BUILDER_MODULE } from "../../modules/product-builder"
...
```ts title="src/workflows/steps/delete-product-builder-complementary-products.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { PRODUCT_BUILDER_MODULE } from "../../modules/product-builder"
...
```ts title="src/workflows/steps/prepare-product-builder-addons.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { PRODUCT_BUILDER_MODULE } from "../../modules/product-builder"
...
```ts title="src/workflows/steps/create-product-builder-addons.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { PRODUCT_BUILDER_MODULE } from "../../modules/product-builder"
...
```ts title="src/workflows/steps/delete-product-builder-addons.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { PRODUCT_BUILDER_MODULE } from "../../modules/product-builder"
...
```ts title="src/workflows/upsert-product-builder.ts"
...
1. Try to retrieve the existing product builder using the `useQueryGraphStep`.
   - This step uses [Query](https://docs.medusajs.com/docs/learn/fundamentals/module-links/query) to retrieve data across modules.
2. Use [when-then](https://docs.medusajs.com/docs/learn/fundamentals/workflows/conditions) to check whether the existing product builder was found.
...
3. Use [transform](https://docs.medusajs.com/docs/learn/fundamentals/workflows/variable-manipulation) to extract the product builder ID from either the existing or newly created product builder.

In a workflow, you can't manipulate data or check conditions because Medusa stores an internal representation of the workflow on application startup. Learn more in the [Data Manipulation](https://docs.medusajs.com/docs/learn/fundamentals/workflows/variable-manipulation) and [Conditions](https://docs.medusajs.com/docs/learn/fundamentals/workflows/conditions) documentation.
...
```ts title="src/api/admin/products/[id]/builder/route.ts"
import { 
...
In the route handler function, you resolve [Query](https://docs.medusajs.com/docs/learn/fundamentals/module-links/query) to retrieve the product builder configuration for the specified product ID. You also retrieve its custom fields, complementary products, and addon products.
...
```ts title="src/api/admin/products/complementary/route.ts"
...
2. An object of [Request Query Configurations](https://docs.medusajs.com/docs/learn/fundamentals/module-links/query#request-query-configurations). You enable the `isList` option to indicate that the pagination query parameters should be added as query configurations in the `req.queryConfig.pagination` object.
...
```ts title="src/api/admin/products/addons/route.ts"
import { MedusaRequest, MedusaResponse } from "@medusajs/framework"
...
```tsx title="src/admin/components/custom-fields-tab.tsx"
import {
  Button,
  Heading,
  Input,
...
```tsx title="src/admin/components/complementary-products-tab.tsx"
...
```tsx title="src/admin/components/addons-tab.tsx"
import { 
  Heading, 
  Checkbox, 
  createDataTableColumnHelper,
...
You display a [Focus Modal](https://docs.medusajs.com/ui/components/focus-modal) that shows the tabs with each of their content.
...
```tsx title="src/admin/widgets/product-builder-widget.tsx"
...
Since you've defined a link between the product and its builder configuration, you can retrieve the builder configuration of a product by specifying it in the `fields` query parameter of the [List Products API Route](https://docs.medusajs.com/api/store/products/list-products).
...
```ts title="src/lib/util/product-builder.ts"
...
```tsx title="src/modules/products/components/product-builder-config/variant-selector.tsx"
"use client"
...
```tsx title="src/modules/products/components/product-builder-config/index.tsx"
"use client"
...
```tsx title="src/modules/products/components/product-price/index.tsx"
...
```tsx title="src/modules/products/components/product-actions/mobile-actions.tsx"
import { BuilderConfiguration } from "../../../../types/global"
import { convertToLocale } from "@lib/util/money"
import { hasProductBuilder } from "@lib/util/product-builder"
```
...
- [acquireLockStep](https://docs.medusajs.com/references/medusa-workflows/steps/acquireLockStep): Acquires a lock on the cart to prevent concurrent modifications.
- [addToCartWorkflow](#addToCartWorkflow): Adds the product to the cart.
- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Get cart with items details.
- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Get updated cart details.
- [releaseLockStep](https://docs.medusajs.com/references/medusa-workflows/steps/releaseLockStep): Releases the lock on the cart.
...
```ts title="src/workflows/steps/validate-product-builder-configuration.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
...
After that, you use the [addToCartWorkflow](https://docs.medusajs.com/references/medusa-workflows/addToCartWorkflow) to add the main product to the cart.
...
```ts title="src/api/store/carts/[id]/product-builder/route.ts"
...
```tsx title="src/modules/cart/components/cart-item-select/index.tsx"
"use client"
...
```tsx title="src/modules/common/components/delete-button/index.tsx"
...
```tsx title="src/modules/common/components/line-item-options/index.tsx"
...
```tsx title="src/modules/common/components/line-item-price/index.tsx"
return (
  <div>
    {/* ... */}
    <span
...
```tsx title="src/modules/common/components/line-item-unit-price/index.tsx"
return (
...
```tsx title="src/modules/cart/templates/preview.tsx"
...
```tsx title="src/modules/layout/components/cart-dropdown/index.tsx"
const CartDropdown = ({
  cart: cartState,
}: {
...
- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Retrieve cart details.
...
- [deleteLineItemsWorkflow](https://docs.medusajs.com/references/medusa-workflows/deleteLineItemsWorkflow): Delete line items from the cart.
- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Retrieve the updated cart details.
...
```ts title="src/workflows/remove-product-builder-from-cart.ts"
import { 
  createWorkflow, 
  WorkflowResponse,
  transform,
...
```tsx title="src/admin/widgets/order-builder-details-widget.tsx"
import { defineWidgetConfig } from "@medusajs/admin-sdk"

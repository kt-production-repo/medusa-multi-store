...
Do not set this option when deploying to [Cloud](https://docs.medusajs.com/cloud/projects/prerequisites). Cloud automatically manages the database connection and SSL configuration. Setting it manually may cause deployment or database migration failures.
...
- dynamicPassword: (\`() => string\` | \`() => Promise\<string>\`) A function that returns a password (or a promise resolving to a password) called for each new database connection. Use this for short-lived credentials, such as AWS RDS IAM authentication tokens, that must be refreshed per connection. Available since v2.18.0.
- expirationChecker: (\`() => boolean\`) A function that returns \`true\` when a connection's credentials have expired. When this returns \`true\`, the connection pool creates a new connection and calls \`dynamicPassword\` to obtain fresh credentials. Available since v2.18.0.
...
- entity\_type: (\`string\`) The type of entity to verify. For example, "email" for email verification.
- auth\_provider: (\`string\`) The auth provider ID that the verification is associated with. For example, "emailpass" for email/password authentication.
...
Do not set this option when deploying to [Cloud](https://docs.medusajs.com/cloud/projects/prerequisites). Cloud automatically manages Redis provisioning. Setting it manually may cause deployment failures.

...
When you install a Medusa application, you get a fully-fledged commerce platform with a Framework for customization. The Medusa application's commerce features are built around [Commerce Modules](https://docs.medusajs.com/resources/commerce-modules) that are available out-of-the-box.
...
Refer to the [Modules documentation](https://docs.medusajs.com/docs/learn/fundamentals/modules) to learn more.
...
Refer to the [Data Models documentation](https://docs.medusajs.com/docs/learn/fundamentals/modules#1-create-data-model) to learn more.
...
```ts title="src/modules/preorder/models/preorder-variant.ts"
...
Learn more about defining data model properties in the [Property Types documentation](https://docs.medusajs.com/docs/learn/fundamentals/data-models/properties).
...
```ts title="src/modules/preorder/models/preorder.ts"
import { model } from "@medusajs/utils"
...
Refer to the [Module Service documentation](https://docs.medusajs.com/docs/learn/fundamentals/modules#2-create-service) to learn more.
...
Refer to the [Migrations documentation](https://docs.medusajs.com/docs/learn/fundamentals/modules#5-generate-migrations) to learn more.
...
Refer to the [Module Isolation documentation](https://docs.medusajs.com/docs/learn/fundamentals/modules/isolation) to learn more.
...
Refer to the [Module Links](https://docs.medusajs.com/docs/learn/fundamentals/module-links) documentation to learn more about defining links.
...
1. A [workflow](https://docs.medusajs.com/docs/learn/fundamentals/workflows) with steps that define the business logic of the feature.
2. An [API route](https://docs.medusajs.com/docs/learn/fundamentals/api-routes) that exposes the workflow's functionality to client applications.
...
Refer to the [Workflows documentation](https://docs.medusajs.com/docs/learn/fundamentals/workflows) to learn more.

The workflow you'll build will have the following steps:

- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Confirm that the product variant exists.
- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Try to retrieve existing pre-order configurations.
...
If you get a type error on resolving the Preorder Module, run the Medusa application once with the `npm run dev` or `yarn dev` command to generate the necessary type definitions, as explained in the [Automatically Generated Types guide](https://docs.medusajs.com/docs/learn/fundamentals/generated-types).

```ts title="src/workflows/steps/update-preorder-variant.ts"
...
2. An async function that receives two parameters:
   ...
   - An object that has properties including the [Medusa container](https://docs.medusajs.com/docs/learn/fundamentals/medusa-container), which is a registry of Framework and commerce tools that you can access in the step.
...
```ts title="src/workflows/steps/create-preorder-variant.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { PREORDER_MODULE } from "../../modules/preorder"
...
```ts title="src/workflows/upsert-product-variant-preorder.ts"
...
1. Try to retrieve the variant's details to confirm it exists. The [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep) uses [Query](https://docs.medusajs.com/docs/learn/fundamentals/module-links/query) which allows you to retrieve data across modules.
...
3. Use [when-then](https://docs.medusajs.com/docs/learn/fundamentals/workflows/conditions) to check if there are existing pre-order configurations.
...
`when-then` and `transform` allow you to access the values of data during execution. Learn more in the [Data Manipulation](https://docs.medusajs.com/docs/learn/fundamentals/workflows/variable-manipulation) and [When-Then](https://docs.medusajs.com/docs/learn/fundamentals/workflows/conditions) documentation.
...
Refer to the [API routes](https://docs.medusajs.com/docs/learn/fundamentals/api-routes) documentation to learn more about them.
...
To validate the body parameters of requests sent to the API route, you need to apply a [middleware](https://docs.medusajs.com/docs/learn/fundamentals/api-routes/middlewares).
...
Refer to the [Middlewares](https://docs.medusajs.com/docs/learn/fundamentals/api-routes/middlewares) documentation to learn more.
...
- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Retrieve the pre-order configurations of a variant.
...
```ts title="src/workflows/steps/disable-preorder-variant.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { PREORDER_MODULE } from "../../modules/preorder"
import { 
  PreorderVariantStatus,
...
```ts title="src/workflows/disable-preorder-variant.ts"
...
Refer to the [Admin Development](https://docs.medusajs.com/docs/learn/fundamentals/admin) documentation to learn more.
...
```ts title="src/admin/lib/types.ts"
...
Refer to the [Admin Development Tips](https://docs.medusajs.com/docs/learn/fundamentals/admin/tips#send-requests-to-api-routes) documentation to learn more.

Create the file `src/admin/hooks/use-preorder-variant.ts` with the following content:

```ts title="src/admin/hooks/use-preorder-variant.ts"
...
- `preorderVariant`: The pre-order variant. It's retrieved from Medusa's [Retrieve Variant](https://docs.medusajs.com/api/admin/products/get-variant) API route, which allows expanding linked records using the `fields` query parameter.
...
To show the pre-order configuration form, you use the [Drawer](https://docs.medusajs.com/ui/components/drawer) component from Medusa UI.
...
- [completeCartWorkflow](https://docs.medusajs.com/references/medusa-workflows/completeCartWorkflow): Complete the cart with pre-order items.
- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Retrieve existing preorders of the order for idempotency.
- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Retrieve all line items in the cart.
- [retrievePreorderItemIdsStep](#retrievePreorderItemIdsStep): Retrieve the IDs of pre-order variants in the cart.
- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Retrieve the created order.
...
```ts title="src/workflows/steps/create-preorders.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { PREORDER_MODULE } from "../../modules/preorder"
...
```ts title="src/workflows/complete-cart-preorder.ts"
import { 
  createWorkflow, 
  when, 
  WorkflowResponse,
...
1. Acquire a lock on the cart using the [acquireLockStep](https://docs.medusajs.com/references/medusa-workflows/steps/acquireLockStep) to prevent concurrent modifications.
2. Complete the cart using the [completeCartWorkflow](https://docs.medusajs.com/references/medusa-workflows/completeCartWorkflow) as a step. This is Medusa's cart completion logic.
3. Retrieve existing pre-orders of the created order using the [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep).
   - This is essential for idempotency, ensuring that pre-orders are not created multiple times if the workflow is retried.
4. Retrieve all line items in the cart using the [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep).
5. Retrieve the IDs of the pre-order variants in the cart using the `retrievePreorderItemIdsStep`.
6. Use [when-then](https://docs.medusajs.com/docs/learn/fundamentals/workflows/conditions) to check if there are no existing pre-orders and if there are pre-order items in the cart.
   - If the condition is met, you create `Preorder` records for the pre-order items using the `createPreordersStep`.
7. Retrieve the created order using the [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep).
8. Release the lock on the cart using the [releaseLockStep](https://docs.medusajs.com/references/medusa-workflows/steps/releaseLockStep).
...
You do this using [workflow hooks](https://docs.medusajs.com/docs/learn/fundamentals/workflows/workflow-hooks). A workflow hook is a point in a workflow where you can inject custom functionality as a step function.
...
```ts title="src/workflows/hooks/validate-cart.ts"
...
Refer to the [Workflow Hooks](https://docs.medusajs.com/docs/learn/fundamentals/workflows/workflow-hooks) documentation to learn more.
...
```ts title="src/lib/util/is-preorder.ts"
...
```tsx title="src/modules/cart/templates/items.tsx"
import { StoreCartLineItemWithPreorder } from "../../../types/global"
```
...
```ts title="src/admin/hooks/use-preorders.ts"
...
```tsx title="src/admin/widgets/preorder-widget.tsx"
import { defineWidgetConfig } from "@medusajs/admin-sdk"
...
In this step, you'll create a workflow that automatically fulfills a pre-order. Then, you'll execute the workflow in a [scheduled job](https://docs.medusajs.com/docs/learn/fundamentals/scheduled-jobs) that runs every day.

### a. Create Auto Fulfill Pre-order Workflow

The workflow that automatically fulfills a pre-order has the following steps:

- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Retrieve the Medusa order details.
- [retrieveItemsToFulfillStep](#retrieveItemsToFulfillStep): Retrieve the items to fulfill in the order.
- [createOrderFulfillmentWorkflow](https://docs.medusajs.com/references/medusa-workflows/createOrderFulfillmentWorkflow): Create a fulfillment for the Medusa order.
- [updatePreordersStep](#updatePreordersStep): Update the status of the pre-order.
- [emitEventStep](https://docs.medusajs.com/references/helper-steps/emitEventStep): Emit an event indicating that a pre-order was fulfilled.
...
```ts title="src/workflows/steps/retrieve-items-to-fulfill.ts"
...
```ts title="src/workflows/steps/update-preorders.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
...
- Retrieve the Medusa order using the [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep).
- Retrieve the items to fulfill in the order using the `retrieveItemsToFulfillStep`.
- Create a fulfillment for the Medusa order using the [createOrderFulfillmentWorkflow](https://docs.medusajs.com/references/medusa-workflows/createOrderFulfillmentWorkflow) as a step.
- Update the pre-order status to `FULFILLED` using the `updatePreordersStep`.
- Emit an event indicating that a pre-order was fulfilled using the [emitEventStep](https://docs.medusajs.com/references/helper-steps/emitEventStep).
...
To do that, you can use Medusa's [capturePaymentWorkflow](https://docs.medusajs.com/references/medusa-workflows/capturePaymentWorkflow) to capture the payment for the order in the workflow.
...
Refer to the [Scheduled Jobs](https://docs.medusajs.com/docs/learn/fundamentals/scheduled-jobs) documentation to learn more.
...
For example, you may want to send a notification to the customer when their pre-order is fulfilled. You can do this by creating a [subscriber](https://docs.medusajs.com/docs/learn/fundamentals/events-and-subscribers).
...
```ts title="src/subscribers/preorder-notification.ts"
import type {
  SubscriberArgs,
  SubscriberConfig,
...
In the subscriber, you retrieve the details of the order and pre-order variants. Then, you use the [Notification Module](https://docs.medusajs.com/resources/infrastructure-modules/notification) to send a notification.

Notice that the `createNotifications` method receives a `channel` property for a notification. This indicates which [Notification Module Provider](https://docs.medusajs.com/resources/infrastructure-modules/notification#what-is-a-notification-module-provider) to send the notification with.

The `feed` channel is useful for debugging, as it logs the notification in the terminal. To send an email, you can instead [set up a provider like SendGrid](https://docs.medusajs.com/resources/integrations#notification) and change the channel to `email`.
...
Learn more about sending notifications in the [Notification Module](https://docs.medusajs.com/resources/infrastructure-modules/notification) documentation.
...
```ts title="src/workflows/cancel-preorders.ts"
import { InferTypeOf } from "@medusajs/framework/types"
...
```ts title="src/subscribers/order-canceled.ts"
...
- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Retrieve the order's items with their pre-order variants.
- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Retrieve the pre-orders of the order.
...
```ts title="src/workflows/steps/retrieve-preorder-updates.ts"
...
```ts title="src/workflows/handle-order-edit.ts"
import { createWorkflow, transform, WorkflowResponse } from "@medusajs/framework/workflows-sdk"
...
1. Retrieve the order details using the [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep).
2. Retrieve the pre-orders of the order using the [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep).
...
6. Emit the `preorder.cancelled` event for the pre-orders that were canceled using the [emitEventStep](https://docs.medusajs.com/references/helper-steps/emitEventStep).
   - You can handle the event similar to the [preorder.fulfilled](#optional-send-notification-to-customer) event to notify the customer.
...
3. In the order edit form, you can add or remove items from the order.
   - Learn more about the order edit form in the [user guide](https://docs.medusajs.com/user-guide/orders/edit).

...
When you install a Medusa application, you get a fully-fledged commerce platform with a Framework for customization. The Medusa application's commerce features are built around [Commerce Modules](https://docs.medusajs.com/resources/commerce-modules), which are available out-of-the-box. These features include authentication with custom providers and for custom user or actor types.
...
In Medusa, you integrate custom authentication providers by creating an [Authentication Module Provider](https://docs.medusajs.com/resources/commerce-modules/auth/auth-providers). Then, you can use that provider to authenticate users using custom logic.
...
- `container`: The [module's container](https://docs.medusajs.com/docs/learn/fundamentals/modules/container) that contains Framework resources available to the module. You access the following resources:
  - `logger`: A [Logger](https://docs.medusajs.com/docs/learn/debugging-and-testing/logging) class to log debug messages.
  - `event_bus`: The [Event Module](https://docs.medusajs.com/resources/infrastructure-modules/event)'s service to emit events.
...
Refer to the [Create Auth Module Provider](https://docs.medusajs.com/references/auth/provider) guide for detailed information about the methods.
...
When a customer (or another actor type) registers in your application, they must also have an [auth identity](https://docs.medusajs.com/resources/commerce-modules/auth/auth-identity-and-actor-types) that allows them to login.
...
Medusa uses the `register` method to create an auth identity that will be associated with the customer when they register. You can learn more in the [Authentication Flows](https://docs.medusajs.com/resources/commerce-modules/auth/auth-flows) documentation.
...
- `authIdentityProviderService`: A service injected by the [Auth Module](https://docs.medusajs.com/resources/commerce-modules/auth) that allows you to manage auth identities.

The method receives other parameters, which you can find in the [Create Auth Module Provider](https://docs.medusajs.com/references/auth/provider#register) guide.
...
The method receives other parameters, which you can find in the [Create Auth Module Provider](https://docs.medusajs.com/references/auth/provider#authenticate) guide.
...
The method receives other parameters, which you can find in the [Create Auth Module Provider](https://docs.medusajs.com/references/auth/provider#validatecallback) guide.
...
Next, you'll register the customer using the [Register Customer](https://docs.medusajs.com/api/store/customers/register-customer) API route. You'll pass the registration token you received in the previous step in the header of this request.
...
- The customer details in the request body with the ones you want to use. Use the same phone number you used in the previous step.
  - You pass the email because it's required by the [Register Customer](https://docs.medusajs.com/api/store/customers/register-customer) API route. You set it to the phone number with a `gmail.com` domain.
...
Next, you'll authenticate the customer using the [Authenticate Customer](https://docs.medusajs.com/api/store/auth/authenticate-customer) API route. This would send the customer an OTP to their phone number (which you'll implement in the next step).
...
So, to verify the OTP, you'll send a request to the [Verify Callback API route](https://docs.medusajs.com/api/store/auth/validate-authentication-callback):
...
You can use this token to authenticate the customer in the application. For example, you can use the token to [retrieve the customer's details](https://docs.medusajs.com/api/store/customers/get-customer).
...
Similar to the Auth Module, the [Notification Module](https://docs.medusajs.com/resources/infrastructure-modules/notification) allows registering custom providers to send notifications, such as SMS or email.
...
- `container`: The [module's container](https://docs.medusajs.com/docs/learn/fundamentals/modules/container) that contains Framework resources available to the module. You don't need to access any resources for this provider.
- `options`: Options that are passed to the module provider when it's registered in Medusa's configurations. You define the following option:
...
Refer to the [Create Notification Module Provider](https://docs.medusajs.com/references/notification-provider-module) guide for detailed information about the methods.
...
The method receives other parameters, which you can find in the [Create Notification Module Provider](https://docs.medusajs.com/references/notification-provider-module#send) guide.
...
You can listen to events in a [subscriber](https://docs.medusajs.com/docs/learn/fundamentals/events-and-subscribers). A subscriber is an asynchronous function that listens to events to perform actions when the event is emitted.
...
Refer to the [Events and Subscribers](https://docs.medusajs.com/docs/learn/fundamentals/events-and-subscribers) documentation to learn more.
...
- `container`: The [Medusa container](https://docs.medusajs.com/docs/learn/fundamentals/medusa-container), which you can use to resolve Framework and commerce resources.
...
In this step, you'll customize the [Next.js Starter Storefront](https://docs.medusajs.com/resources/nextjs-starter) to allow customers to authenticate using their phone number and OTP.
...
In the function, you use the [JS SDK](https://docs.medusajs.com/resources/js-sdk), which is configured within the Next.js Starter Storefront, to send a request to the `/auth/customer/phone-auth` API route. You pass the phone number in the request body.
...
Then, after setting the registration token in the cookies, you create a customer using the [Create Customer](https://docs.medusajs.com/api/store/customers/register-customer) API route. You pass the following properties in the request body:
...
```tsx title="src/modules/account/components/otp/index.tsx"
"use client"
...
```tsx title="src/modules/account/components/register-phone/index.tsx"
"use client"
...
```tsx title="src/modules/account/components/login-phone/index.tsx"
"use client"
...
```tsx title="src/app/[countryCode]/(main)/account/@dashboard/profile/page.tsx"
...
In this section, you'll add a [middleware](https://docs.medusajs.com/docs/learn/fundamentals/api-routes/middlewares) to the `/store/customers/me` API route that prevents customers from updating their phone number.
...
Learn more in the [Middlewares](https://docs.medusajs.com/docs/learn/fundamentals/api-routes/middlewares) documentation.

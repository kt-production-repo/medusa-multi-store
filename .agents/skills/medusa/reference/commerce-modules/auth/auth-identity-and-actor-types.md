...
The [AuthIdentity data model](https://docs.medusajs.com/references/auth/models/AuthIdentity) represents a user registered by an [authentication provider](https://docs.medusajs.com/resources/commerce-modules/auth/auth-providers). When a user is registered using an authentication provider, the provider creates a record of `AuthIdentity`.
...
When you protect routes with the [authenticate middleware](https://docs.medusajs.com/docs/learn/fundamentals/api-routes/protected-routes), you specify in its first parameter the actor type that must be authenticated to access the API routes.
...
Learn how to create a custom actor type in the [Create Manager Actor Type guide](https://docs.medusajs.com/resources/commerce-modules/auth/create-actor-type).

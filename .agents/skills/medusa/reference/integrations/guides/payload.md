...
```ts title="src/collections/Media.ts"
...
```ts title="src/collections/ProductOptions.ts"
import { CollectionConfig } from "payload"
...
```ts title="src/collections/OptionValues.ts"
import { CollectionConfig } from "payload"
...
```ts title="src/collections/Products.ts"
import { CollectionConfig } from "payload"
...
Now that Payload is set up in the Next.js Starter Storefront, you'll create a Payload [Module](https://docs.medusajs.com/docs/learn/fundamentals/modules) to integrate it with Medusa.
...
```ts title="src/modules/payload/types.ts"
...
```ts title="src/modules/payload/service.ts"
import {
...
1. The [Module container](https://docs.medusajs.com/docs/learn/fundamentals/modules/container) that allows you to resolve module and Framework resources. You don't need to resolve any resources in this module, so you can leave it empty.
...
```ts title="src/modules/payload/index.ts"
...
```ts title="src/links/product-payload.ts"
...
- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Retrieve the product options from Medusa
- [createPayloadItemsStep](#createPayloadItemsStep): Create the product options in Payload
- [createPayloadItemsStep](#createPayloadItemsStep): Create the option values in Payload
- [updatePayloadItemsStep](#updatePayloadItemsStep): Update product options to reference the values
...
If you get a type error on resolving the Payload Module, run the Medusa application once with the `npm run dev` or `yarn dev` command to generate the necessary type definitions, as explained in the [Automatically Generated Types guide](https://docs.medusajs.com/docs/learn/fundamentals/generated-types).

```ts title="src/workflows/steps/create-payload-items.ts"
...
```ts title="src/workflows/steps/update-payload-items.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
...
```ts title="src/workflows/create-payload-product-options.ts"
...
```ts title="src/subscribers/product-option-created.ts"
...
- [retrievePayloadItemsStep](#retrievePayloadItemsStep): Retrieve the product options from Payload
- [retrievePayloadItemsStep](#retrievePayloadItemsStep): Retrieve the option values from Payload to reference them in variants
...
```ts title="src/workflows/steps/retrieve-payload-items.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { PAYLOAD_MODULE } from "../../modules/payload"
...
```ts title="src/workflows/create-payload-products.ts"
...
2. Check which product options already exist in Payload by retrieving them using `retrievePayloadItemsStep`.
   - To manipulate data in a workflow, you need to use the `transform` function. Learn more in the [Data Manipulation](https://docs.medusajs.com/docs/learn/fundamentals/workflows/variable-manipulation) documentation.
3. Create missing product options using the `createPayloadProductOptionsWorkflow` if any options don't exist yet.
   - To run steps based on a condition, you need to use the `when` function. Learn more in the [Conditions in Workflows](https://docs.medusajs.com/docs/learn/fundamentals/workflows/conditions) documentation.
...
- An [API route](https://docs.medusajs.com/docs/learn/fundamentals/api-routes) that emits a `products.sync-payload` event.
- A [subscriber](https://docs.medusajs.com/docs/learn/fundamentals/events-and-subscribers) that listens to the `products.sync-payload` event and executes the `createPayloadProductsWorkflow`.
- A [setting page](https://docs.medusajs.com/docs/learn/fundamentals/admin/ui-routes) in the Medusa Admin that allows admin users to trigger the product creation in Payload.
...
```ts title="src/api/admin/payload/sync/[collection]/route.ts"
...
In the function, you resolve the [Event Module](https://docs.medusajs.com/resources/infrastructure-modules/event)'s service and emit a `{collection}.sync-payload` event, where `{collection}` is the collection slug passed in the request.
...
```ts title="src/subscribers/products-sync-payload.ts"
import { SubscriberArgs, type SubscriberConfig } from "@medusajs/framework"
...
In the subscriber, you use [Query](https://docs.medusajs.com/docs/learn/fundamentals/module-links/query) to retrieve all products from Medusa.
...
To send requests from your Medusa Admin customizations to the Medusa server, you need to initialize the [JS SDK](https://docs.medusajs.com/resources/js-sdk).
...
Refer to the [JS SDK documentation](https://docs.medusajs.com/resources/js-sdk) to learn more about initializing the SDK.

#### Create the Setting Page

A setting page is a [UI route](https://docs.medusajs.com/docs/learn/fundamentals/admin/ui-routes) that adds a custom page to the Medusa Admin under the Settings section. The UI route is a React component that renders the page's content.

Refer to the [UI Routes](https://docs.medusajs.com/docs/learn/fundamentals/admin/ui-routes) documentation to learn more.

To create the setting page, create the file `src/admin/routes/settings/payload/page.tsx` with the following content:

```tsx title="src/admin/routes/settings/payload/page.tsx"
...
```ts title="src/lib/util/payload-images.ts"
import { StoreProductWithPayload } from "../../types/global"
...
```tsx title="src/modules/products/components/image-gallery/index.tsx"
...
```ts title="src/workflows/steps/delete-payload-items.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { PAYLOAD_MODULE } from "../../modules/payload"
...
```ts title="src/workflows/delete-payload-products.ts"
import { createWorkflow, transform, WorkflowResponse } from "@medusajs/framework/workflows-sdk"
...
- [retrievePayloadItemsStep](#retrievePayloadItemsStep): Retrieve the option values from Payload to reference them in the variant
...
```ts title="src/workflows/create-payload-product-variant.ts"
import { createWorkflow, transform, when, WorkflowResponse } from "@medusajs/framework/workflows-sdk"
import { useQueryGraphStep } from "@medusajs/medusa/core-flows"
import { PayloadCollectionItem, PayloadUpsertData } from "../modules/payload/types"
import { retrievePayloadItemsStep } from "./steps/retrieve-payload-items"
...
```ts title="src/workflows/update-payload-product-variants.ts"
import { createWorkflow, transform, when, WorkflowResponse } from "@medusajs/framework/workflows-sdk"
import { useQueryGraphStep } from "@medusajs/medusa/core-flows"
import { PayloadCollectionItem, PayloadUpsertData } from "../modules/payload/types"
import { retrievePayloadItemsStep } from "./steps/retrieve-payload-items"
...
```ts title="src/workflows/delete-payload-product-variants.ts"
import { createWorkflow, transform, when, WorkflowResponse } from "@medusajs/framework/workflows-sdk"
import { updatePayloadItemsStep } from "./steps/update-payload-items"
import { retrievePayloadItemsStep } from "./steps/retrieve-payload-items"
...
- [retrievePayloadItemsStep](#retrievePayloadItemsStep): Retrieve the product options to delete from Payload
- [deletePayloadItemsStep](#deletePayloadItemsStep): Delete the option from Payload
...
```ts title="src/workflows/delete-payload-product-options.ts"
import { createWorkflow, transform, when, WorkflowResponse } from "@medusajs/framework/workflows-sdk"
import { retrievePayloadItemsStep } from "./steps/retrieve-payload-items"
import { deletePayloadItemsStep } from "./steps/delete-payload-items"
...
```ts title="src/subscribers/option-deleted.ts"
import { SubscriberArgs, type SubscriberConfig } from "@medusajs/framework"
...
```ts title="src/workflows/update-payload-product-options.ts"
import { createWorkflow, transform, when, WorkflowResponse } from "@medusajs/framework/workflows-sdk"
import { useQueryGraphStep } from "@medusajs/medusa/core-flows"
import { retrievePayloadItemsStep } from "./steps/retrieve-payload-items"
import { createPayloadItemsStep } from "./steps/create-payload-items"
...
```ts title="src/subscribers/option-updated.ts"
import { SubscriberArgs, type SubscriberConfig } from "@medusajs/framework"
...
```ts title="src/workflows/update-payload-products.ts"
import { createWorkflow, transform, when, WorkflowResponse } from "@medusajs/framework/workflows-sdk"
import { useQueryGraphStep } from "@medusajs/medusa/core-flows"
import { retrievePayloadItemsStep } from "./steps/retrieve-payload-items"
import { updatePayloadItemsStep } from "./steps/update-payload-items"
